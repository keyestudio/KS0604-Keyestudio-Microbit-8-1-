### 3.2.8 Hovering Hawk

#### 3.2.8.1 Overview

![Img](./media/top1.png)

Here we build a hovering hawk with a Microbit V2 board, a servo expansion board, a 360° LEGO servo and some LEGO parts. We program to control the fluttering of its wings through the Microbit V2 button A/B.

![Img](./media/bottom1.png)

#### 3.2.8.2 Materials Required

![Img](./media/A8-0.png)

#### 3.2.8.3 Installation Steps

![Img](./media/2top.png)

![Img](./media/A8-1.png)

![Img](./media/A8-2.png)

![Img](./media/A8-3.png)

![Img](./media/A8-4.png)

![Img](./media/A8-5.png)

![Img](./media/A8-6.png)

![Img](./media/A8-7.png)

![Img](./media/A8-8.png)

![Img](./media/A8-9.png)

![Img](./media/A8-10.png)

![Img](./media/A8-11.png)

![Img](./media/A8-12.png)

![Img](./media/A8-13.png)

![Img](./media/A8-14.png)

![Img](./media/A8-15.png)

![Img](./media/A8-16.png)

![Img](./media/A8-17.png)

![Img](./media/A8-18.png)

![Img](./media/A8-19.png)

![Img](./media/A8-20.png)

![Img](./media/A8-21.png)

![Img](./media/A8-22.png)

![Img](./media/A8-23.png)

![Img](./media/A8-24.png)

![Img](./media/A8-25.png)

![Img](./media/A8-26.png)

![Img](./media/A8-27.png)

![Img](./media/A8-28.png)

![Img](./media/A8-29.png)

![Img](./media/A8-30.png)

![Img](./media/A8-31.png)

![Img](./media/A8-32.png)

![Img](./media/A8-33.png)

![Img](./media/A8-34.png)

![Img](./media/A8-35.png)

![Img](./media/A8-36.png)

![Img](./media/A8-37.png)

![Img](./media/A8-38.png)

![Img](./media/A8-39.png)

![Img](./media/A8-40.png)

![Img](./media/A8-41.png)

![Img](./media/A8-42.png)

![Img](./media/A8-43.png)

![Img](./media/A8-44.png)

![Img](./media/A8-45.png)

![Img](./media/A08.png)

![Img](./media/wiring8.png)

![Img](./media/A8-46.png)

![Img](./media/A8-47.png)

![Img](./media/A8-48.png)

![Img](./media/2bottom.png)

#### 3.2.8.4 Test Code

![Img](./media/couj08.png)

⚠️ **<span style="color: rgb(255, 76, 65);">Special reminder: You can obtain the sample code through the following two methods.</span>**

##### **Method 1: Drag and drop code blocks to build the sample code**

**1. MakeCode programming environment**

The online version of MakeCode: <u>[MakeCode Editor](https://makecode.microbit.org/#editor)</u>

**2. Extension library**

⚠️ **<span style="color: rgb(255, 76, 65);">Special reminder: </span>** Copy and paste `https://github.com/keyestudio2019/pxt-creative-inventors-kit-master.git` to the search box to import the library.

![Img](./media/Animation-4.gif)

**3. Write sample code**

![Img](./media/dongtu-08.gif)

##### **Method 2: Download the sample code file directly**

**1. Download the code file:**

Click to download the code file: <u>[3_3_8_Hovering_Hawk](./Codes/3_3_8_Hovering_Hawk.hex)</u>

**2. MakeCode programming environment**

The online version of MakeCode: <u>[MakeCode Editor](https://makecode.microbit.org/#editor)</u>

**3. Load the sample code:**

Drag and drop the sample code to the MakeCode editor.

![Img](./media/Animation-0008.gif)

![Img](./media/line1.png)

#### 3.2.8.5 Test Result

![Img](./media/4top.png)

Wire up and power on through micro USB cable, and and connect an external power source (four AAA-1.5V batteries installed in the battery box, ensuring sufficient power).

To download the sample code using an online browser, you need to send the downloaded ".hex" file to the micro:bit V2 board. There are two ways as follows:

**① Download through WebUSB**

For browsers like **Google Chrome / Microsoft Edge**, you need to click “**Connect Device**” to pair the device first. After that, click “**Download**” to load the sample code to the micro:bit V2 board.

![Img](./media/Animation-08.gif)

**② Download without WebUSB**

For browsers like **Safari / Firefox / Other**, download and send the sample code to the micro:bit V2 board.

![Img](./media/Animation-008.gif)

After downloading the sample code, unplug the micro USB cable to the micro:bit V2 board.

Turn the DIP switched to the RGB and ON.

![Img](./media/CBA01.png)

Press button A on the micro:bit V2 board and then release it, the wings flutter slowly, the five SK6812 RGB LEDs emit red light, and the 5×5 LED matrix displays ![Img](./media/ab3.png).

Press button B on the micro:bit V2 board and then release it, the wings flap rapidly, the five SK6812 RGB LEDs emit green light, and the 5×5 LED matrix displays ![Img](./media/ab6.png).

Press buttons A and B on the micro:bit V2 board simultaneously and then release them, the wings stop, and the five SK6812 RGB LEDs and the 5×5 LED matrix turn off.

![Img](./media/dongtu-008.gif)

